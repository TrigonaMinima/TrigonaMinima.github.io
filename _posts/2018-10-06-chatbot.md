...
### Ashleellaundakaun (Ashleel launda kaun)

*Who is the dirtiest guy?* That is what *ashleel launda kaun* means. GB constantly monitors the chats and maintains a count of expletives used by each user which we can retrieve using ```/ashleellaundakaun``` (later renamed to ```/a```). It'll also use @mention to mark the topper so that a notification goes to that user specifically.
...
3. **Sanitization and Tokenization**: Due to a very basic text sanitization (stripping of white spaces and special characters) and tokenization (simple split on spaces and other punctuation) there were many unwanted corrections.
    ...
    3. Corrections for the numbers and words starting or ending with numbers. Example, it corrected *3C2* (Permutation and Combination notation for choosing 2 out of 3) to *32*. Removed words with numbers and numbers from the vocabulary.
